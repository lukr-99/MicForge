...
/// <summary>Brick-wall-ish safety limiter (instant attack, smooth release).</summary>
public sealed class Limiter : IAudioProcessor
{
    ...
    public void Process(float[] buffer, int offset, int count)
    {
        ...
        for (int i = offset; i < offset + count; i++)
        {
            double x = buffer[i];
            double a = Math.Abs(x);
            double desired = a > ceil ? ceil / a : 1.0;
            if (desired < _gain) _gain = desired;            // instant attack
            else _gain = desired + (_gain - desired) * rel;  // smooth release
